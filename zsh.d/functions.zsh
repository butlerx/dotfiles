...
if [[ $IS_MAC -eq 1 ]]; then
	...
	ips() {
		foo=$(
			/Users/mark/bin/getip.py
			/Users/mark/bin/getip.py en0
			/Users/mark/bin/getip.py en1
		)
		echo "$foo"
	}
...
haste() {
	...
	if (($# == 0)) && [[ $(printf "%s" "$0" | wc -c) -gt 0 ]]; then
		...
		printf "Usage: %s FILE\\nUpload contents of plaintext document to hastebin.\\n\\nInvocation with no arguments takes input from stdin or pipe.\\nTerminate stdin by EOF (Ctrl-D)." "$0"
		return 1
	elif [[ -e $1 && ! -f $1 ]]; then
	...
	if output=$(curl -# -f -XPOST "https://hastebin.com/documents" -d"$contents") && [[ $output =~ \"key\" ]]; then
